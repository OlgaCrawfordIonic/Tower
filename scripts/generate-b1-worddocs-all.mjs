// scripts/generate-b1-worddocs-all.mjs
...
// INPUT: your existing input file
const INPUT_JSON_PATH = path.resolve(
  __dirname,
  '../src/assets/b1-words-openai-input.json'
);

// OUTPUT: full output and a progress file
const OUTPUT_JSON_PATH = path.resolve(
  __dirname,
  '../src/assets/b1-words-openai-output-all.json'
);
const PROGRESS_JSON_PATH = path.resolve(
  __dirname,
  '../src/assets/b1-words-openai-output-progress.json'
);
...
const openai = new OpenAI({
  apiKey,
  timeout: 500000,   // you already set this high
  maxRetries: 2,      // client-side retries for transient issues
});

// ---------- SYSTEM PROMPT (your current version) ----------
...
async function saveProgress(wordsArray) {
  ...
  console.log('Progress saved to', PROGRESS_JSON_PATH);
}
...
async function main() {
  ...
  const allWords = input.words;
  console.log('Total words in input:', allWords.length);
  ...
  for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
    ...
    console.log(
      `Processing batch ${batchIndex + 1}/${totalBatches} (words ${start}–${end - 1})`
    );

    let attempt = 0;
    let success = false;
    while (attempt < MAX_RETRIES_PER_BATCH && !success) {
      attempt++;
      try {
        console.log(`Calling OpenAI (attempt ${attempt}) for ${batch.length} words...`);
        const batchDocs = await callOpenAIForBatch(batch);
        allDocs.push(...batchDocs);
        success = true;
        console.log(`Batch ${batchIndex + 1} succeeded, now have ${allDocs.length} WordDocs total.`);

        // Save progress after each successful batch
        await saveProgress(allDocs);
      } catch (err) {
        console.error(`Batch ${batchIndex + 1} failed on attempt ${attempt}:`, err);
        if (attempt >= MAX_RETRIES_PER_BATCH) {
          console.error(
            `Giving up on batch ${batchIndex + 1} after ${MAX_RETRIES_PER_BATCH} attempts.`
          );
          // You can choose to throw here to stop everything
          throw err;
        } else {
  ...
  // Final full output
  const finalPayload = { words: allDocs };
  await fs.writeFile(
    OUTPUT_JSON_PATH,
  ...
  console.log('Saved full output to', OUTPUT_JSON_PATH);
}
