...
import {  TopicUsage } from '../../data/lexamatewords.model';
...
@Component({
  selector: 'app-importfirebaseold',
  templateUrl: './importfirebaseold.page.html',
  styleUrls: ['./importfirebaseold.page.scss'],
  standalone: true,
  imports: [ CommonModule, FormsModule, IonicModule, RouterLink]
})
export class ImportfirebaseoldPage {
...
private mergeWordDocsKeepingLessons(existing: WordDoc, incoming: WordDoc): WordDoc {
  ...
  (existing.topics || []).forEach(exTopic => {
    ...
    const examples: TopicUsage['examples'] = inTopic?.examples ?? exTopic.examples ?? {
  'en-GB': [],
  'en-US': [],
};
...
// --------- PER-WORD VALIDATION ----------
  ...
  try {
    ...
    for (const [lemma, incoming] of incomingMap.entries()) {
      const ref: DocumentReference = doc(db, 'EnglishB1words', wordDocId(lemma));
      ...
      // Firestore batch limit is 500 writes — commit in chunks to be safe
      if (batchOps >= 450) {
        await batch.commit();
        batch = writeBatch(db);
        batchOps = 0;
    ...
    this.importSuccess = `Created ${created} NEW word(s). Skipped ${skippedExisting} existing word(s).`;
  } catch (err: any) {
    console.error('Create-new failed', err);
    this.importError = 'Create-new failed. See console for details.';
  } finally {
    this.importing = false;
  }
}
